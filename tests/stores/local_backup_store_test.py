...
@pytest.fixture
def mock_backup_store(fs_path):
    backup_name = "fake_backup"
    with (
        mock.patch("backuppy.stores.local_backup_store.BackupStore"),
        staticconf.testing.PatchConfiguration(
            {"protocol": {"location": f"{fs_path}/fake/path"}},
...
def test_save(fs_path, caplog, mock_backup_store):
    with (
...
def test_load(fs_path, mock_backup_store):
