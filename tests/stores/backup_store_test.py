...
def test_unlock(fs_path, backup_store, manifest_exists):
    os.makedirs(f"{fs_path}/my/private")
    with open(f"{fs_path}/my/private/key", "w") as f:
        f.write("THIS IS VERY SECRET")
    shutil.rmtree(get_scratch_dir(), ignore_errors=True)
...
def test_rotate_manifests(backup_store, max_manifest_versions):
    backup_store._query.return_value = [
        "manifest.1234",
        "manifest.1235",
        "manifest.1236",
    ]
...
def test_cleanup_and_exit_no_store(backup_store):
    ...
    assert mock_signal.call_args_list == [mock.call(signal.SIGINT, signal.SIG_IGN)]
...
def test_cleanup_and_exit(backup_store, side_effect):
    ...
    with (
        mock.patch("backuppy.stores.backup_store._UNLOCKED_STORE", backup_store),
        mock.patch("backuppy.stores.backup_store.signal.signal") as mock_signal,
        pytest.raises(SystemExit),
    ):
        _cleanup_and_exit(signal.SIGINT, mock.Mock(), True, True)
...
def test_unregister_store():
    with (
        mock.patch("backuppy.stores.backup_store._UNLOCKED_STORE"),
        mock.patch("backuppy.stores.backup_store.signal.signal") as mock_signal,
    ):
        _unregister_store()
