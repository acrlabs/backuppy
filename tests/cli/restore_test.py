...
def test_parse_destination(fs_path):
    assert _parse_destination("foo/bar", "fake_backup1") == (
        "/code/foo/bar/fake_backup1",  # inside the docker container
    ...
    assert _parse_destination(None, "fake_backup1") == (
        "/code/fake_backup1",  # inside the docker container
        "./fake_backup1",
    )
...
def test_main(retval, sha, entries):
    ...
    with (
        ...
        if entries and retval:
            assert mock_restore.call_args_list == [
                mock.call(
                    entries,
