...
@pytest.fixture
def mock_io_iter(fs_path):
    yield IOIter(f"{fs_path}/foo", block_size=2)


@pytest.fixture
def foo_contents(fs_path):
    contents = b"asdfhjklqwerty"
    with open(f"{fs_path}/foo", "wb") as f:
        f.write(contents)
    yield contents


def test_tmp_io_iter(fs_path):
...
def test_context_manager(mock_io_iter):
    with mock.patch("backuppy.io.TemporaryFile") as mock_tmp_file, mock_io_iter:
        ...
        assert os.path.exists(mock_io_iter.filename)
...
def test_writer(mock_io_iter):
    with open(mock_io_iter.filename, "wb") as f:
        f.write(b"This data will get overwritten")
    contents = b"asdfhjlkqwerty"
    with mock_io_iter:
        writer = mock_io_iter.writer()
        next(writer)
        writer.send(contents)
    with open(mock_io_iter.filename, "rb") as f:
...
def test_writer_tmp_file(block_size):
    contents = b"asdfhjlkqwerty"
    with (
        IOIter(None, block_size=block_size) as tmp,
...
def test_check_mtime(mock_io_iter):
    with mock_io_iter:
        time.sleep(1)
        with (
            open(mock_io_iter.filename, "wb") as f,
            pytest.raises(FileChangedException),
        ):
            f.write(b"asdf")
            mock_io_iter._check_mtime()
...
def test_stat(mock_io_iter):
    with mock_io_iter:
        ...
        assert mock_io_iter.mtime == int(os.stat(mock_io_iter.filename).st_mtime)
...
def test_copy(mock_io_iter, foo_contents):
    with mock_io_iter, IOIter("/test/filesystem/bar") as copy:
        io_copy(mock_io_iter, copy)
    with open("/test/filesystem/bar", "rb") as f:
